function doGet(e) {
  try {
    ...
    if (action === 'get_submissions') {
      const data = getPretestSubmissions(e.parameter);
...
// ... existing getDataForForm ...

function saveData(data) {
  const SPREADSHEET_ID = '1kW_G1I7koBf-RAOG-rziRb1_N96h28YF7GH1EwTacYI';
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  
  // Determine Sheet Name based on data.type and data.theme
  const type = data.type || 'pretest';
  const theme = data.theme || 't1';
  let sheetName = `${type}-${theme}`;
...
function getPretestSubmissions(params) {
  const SPREADSHEET_ID = '1kW_G1I7koBf-RAOG-rziRb1_N96h28YF7GH1EwTacYI';
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  
  const type = (params && params.type) ? params.type : 'pretest';
  const theme = (params && params.theme) ? params.theme : 't1';
  const sheetName = `${type}-${theme}`;
  ...
  // Get all data: Row 2 to LastRow, Cols 1 to 8 (Score)
